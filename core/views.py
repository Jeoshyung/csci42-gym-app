...
from .models import WorkoutSession, WorkoutLogging, Exercise, FitnessGoal
...
def index_view(request):
    ...
    start_of_week = now().date() - timedelta(days=now().weekday())  # Start of the current week
    weekly_sessions = WorkoutSession.objects.filter(user=user, date__date__gte=start_of_week)

    total_workouts = weekly_sessions.count()
    total_sets = sum(session.entries.aggregate(total_sets=Sum('sets'))['total_sets'] or 0 for session in weekly_sessions)
    total_reps = sum(session.entries.aggregate(total_reps=Sum('reps'))['total_reps'] or 0 for session in weekly_sessions)
    ...
    for goal in goals:
        ...
        goal.progress_percentage = min(100, (progress_value / goal.target_value) * 100)
    ...
    serialized_goals = json.dumps(
        [{'name': goal.name, 'current_value': goal.current_value, 'target_value': goal.target_value} for goal in goals],
...
def add_goal_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('index')  # Redirect to the index page after adding the goal
    else:
